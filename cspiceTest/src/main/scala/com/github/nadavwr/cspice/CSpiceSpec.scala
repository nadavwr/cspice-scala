...
object CSpiceSpec extends Spec with App {
  trait EarthFixture extends Fixture {
  ...
  test("the earth should rotate from one perihelion to the next") runWith new EarthFixture {
    ...
    assertThat(error <= tolerance,
      s"perihelion error rate $error should be smaller than $tolerance")
  }

  test("the earth should reach aphelion at half a year") runWith new EarthFixture {
    ...
    assertThat(error <= tolerance,
      s"aphelion error rate $error should be smaller than $tolerance")
  }
}
